...
14. As Raja, I want each occurrence to be done, skipped, or missed, so that a deliberate skip never reads as failure.
...
- **pgTAP (CI-only)** is the primary seam: RLS on every new table; plan cascade integrity (horizon hierarchy, no orphan commitments); commitment uniqueness (no duplicate placement of one subject in one plan); habit rule edits from an effective date never rewrite past occurrences; occurrence state transitions; block→task binding invariant; session↔block linkage; attachment subject polymorphism constraints; sweep moves exactly the unfinished set. Prior art: `hooks_contract` (41 assertions), phase_2 tests.
