...
- **Schema (additive migration, existing owner_id + RLS discipline)**:
  - `profiles.time_zone` — IANA zone, non-null with `UTC` fallback. The client sets the owner's actual zone; all Today, ISO-week, Sweep, occurrence-generation, and review boundaries derive from it. Raw database-session `current_date` is never an owner calendar boundary.
  - `identities` — promoted from `goals.identity_role` text.
  - `plans` — one self-referencing table, `horizon ∈ {year, quarter, month, week, day}`, `parent_plan_id`, and `starts_on date not null` as the canonical calendar identity (the first owner-local date of the period). `day_records` absorbed into day plans, so a day plan is addressed by its date. Constraints: `starts_on` must be horizon-aligned (year → Jan 1, quarter → Jan/Apr/Jul/Oct 1, month → day 1, week → ISO Monday, day → any date); `unique (owner_id, horizon, starts_on)`; `parent_plan_id` must be the same owner, the next horizon up, and contain `starts_on` in its period, and is null only for `year`. Today is `(owner, 'day', (now() at time zone profiles.time_zone)::date)` and the ISO week derives from that same owner-local date — deterministic lookups, no scanning, session-time-zone drift, or tie-breaking.
